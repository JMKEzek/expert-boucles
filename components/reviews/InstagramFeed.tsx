...
export function InstagramFeed({ posts, columns = 3 }: InstagramFeedProps) {
    ...
    return (
      <div className="py-48 text-center">
        <p className="text-gris-medium text-xs uppercase tracking-0.2em">
          Publications Instagram temporairement indisponibles
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={`grid w-full grid-cols-2 md:grid-cols-3 ${desktopColumns} gap-px bg-gris-light`}>
      {posts.map((post) => (
        <a
          key={post.id}
          href={post.url || '#'}
          ...
          aria-label={post.caption ? `Voir sur Instagram : ${post.caption}` : 'Voir la publication sur Instagram'}
        >
          <Image
            src={post.image}
            alt={post.caption || 'Réalisation Expert Boucles'}
            fill
